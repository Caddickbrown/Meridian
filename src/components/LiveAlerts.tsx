...
export default function LiveAlerts({ data, onLocate, onWatchFeed }: LiveAlertsProps) {
  ...
  const content = (
    <motion.div
      ...
      className={`glass-panel flex flex-col overflow-hidden pointer-events-auto transition-all duration-300 ${maximized ? 'fixed inset-4 z-[9999] bg-[#0a0a09]/95' : 'shrink-0 h-[500px] max-h-[80vh] resize-y'}`}
    >

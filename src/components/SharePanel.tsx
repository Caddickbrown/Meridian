...
export default function SharePanel({ mapView, activeLayers, mouseCoords }: SharePanelProps) {
  ...
  const generateShareUrl = useCallback(() => {
    ...
    const base = typeof window !== 'undefined' ? window.location.origin : 'https://cooperativesausage.com/meridian';
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="flex gap-2">
              <a
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent('🗺️ Meridian — Global Intelligence Dashboard')}&url=${encodeURIComponent(generateShareUrl())}`}
                target="_blank"
              ...
              <a
                href={`https://reddit.com/submit?url=${encodeURIComponent(generateShareUrl())}&title=${encodeURIComponent('Meridian — Global Intelligence Platform')}`}
                target="_blank"

  ...
  if (!open && !alwaysExpanded) {
    return (
      <button
        ...
        className="flex items-center gap-1.5 glass-panel-sm px-3 py-2 text-[10px] font-mono tracking-[0.1em] text-[var(--text-muted)] hover:text-[var(--gold-primary)] hover:border-[var(--border-active)] transition-all hover:shadow-[0_0_12px_rgba(53,214,181,0.08)]"
      >
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      <div className="flex items-center gap-2 glass-panel px-3 py-2.5 !border-[var(--border-active)] transition-all"
        style={{ boxShadow: '0 0 20px rgba(53,214,181,0.05), inset 0 0 20px rgba(0,0,0,0.2)' }}
      >
      ...
      {results.length > 0 && (
        <div
          ...
          style={{ boxShadow: '0 12px 40px rgba(0,0,0,0.6), 0 0 1px rgba(53,214,181,0.2)' }}
        >
          {results.map((r, i) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-3 py-2.5 transition-colors border-b border-[var(--border-secondary)] last:border-0 flex items-start gap-2.5 ${
                  isSelected ? 'bg-[rgba(53,214,181,0.08)]' : 'hover:bg-[var(--hover-accent)]'
                }`}
              >

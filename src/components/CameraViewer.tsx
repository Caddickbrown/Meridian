...
export default function CameraViewer({ camera, onClose, onLocate }: CameraViewerProps) {
  ...
  return (
    <AnimatePresence>
        ...
        >
          <div className="overflow-hidden h-full flex flex-col bg-black/85 border border-[var(--border-primary)]" style={{ boxShadow: '0 20px 50px rgba(0,0,0,0.9), inset 0 0 30px rgba(0,0,0,0.8)' }}>
            ...
            <div className="flex flex-col border-b border-[var(--border-primary)] bg-black/60 relative z-10">
              {/* Top Meta Bar */}
              <div className="flex items-center justify-between px-3 py-1 border-b border-white/5 text-[9px] font-mono tracking-[0.1em] text-[var(--text-muted)] bg-[var(--hover-accent)]">
              ...
              <div className="flex items-center justify-between px-3 md:px-4 py-2 md:py-3">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="text-[11px] md:text-[12px] font-mono font-bold tracking-[0.1em] truncate text-white uppercase" style={{ textShadow: '0 0 10px rgba(255,255,255,0.3)' }}>{camera.name}</h3>
          ...
          <div className={`relative bg-[#020202] ${fullscreen ? 'flex-1 overflow-hidden' : 'aspect-video max-h-[35vh] md:max-h-none'}`}>
            ...
            {loading && !error && !externalOnly && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/90 z-30">
                <div className="text-center">
                  ...
                  <span className="text-[10px] font-mono tracking-[0.1em]" style={{ color: 'var(--gold-primary)' }}>DECRYPTING FEED...</span>
                </div>
              </div>
            )}

            {view === 'resolving' ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90 z-30 p-4 text-center">
                ...
                <p className="text-[11px] font-mono uppercase tracking-[0.1em]" style={{ color: 'var(--gold-primary)' }}>ACQUIRING UPLINK</p>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90 z-30 p-4 text-center">
                ...
                <p className="text-[11px] font-mono uppercase tracking-[0.1em] text-[var(--text-secondary)]">{gone ? 'CAMERA WITHDRAWN' : 'CAMERA OFFLINE'}</p>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90 z-30 p-4 text-center">
                ...
                <p className="text-[11px] font-mono uppercase tracking-[0.1em]" style={{ color: 'var(--gold-primary)' }}>SECURE FEED ENCRYPTED</p>
                ...
                <a 
                  ...
                  className="mt-4 px-4 py-2 rounded text-[10px] font-mono font-bold tracking-[0.1em] transition-all hover:bg-white/10"
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/90">
                <div className="text-center">
                  ...
                  <span className="text-[10px] font-mono text-red-400 tracking-[0.1em] block mb-1">FEED UNAVAILABLE</span>
            ...
            {!error && !loading && !externalOnly && (
              <div className="absolute top-3 left-3 flex items-center gap-2 bg-black/80 border border-[var(--gold-primary)]/50 px-2 py-1 shadow-[0_0_10px_rgba(0,0,0,0.8)]">
                ...
                <span className="text-[9px] font-mono text-white tracking-[0.1em]">
            ...
            {watchLiveUrl && !error && !externalOnly && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="absolute bottom-3 right-3 z-30 flex items-center gap-1.5 px-3 py-1.5 rounded-sm border text-[10px] font-mono font-bold tracking-[0.1em] transition-all hover:bg-[var(--gold-primary)]/25"
            ...
            <div className="flex items-center justify-between px-3 py-1.5 border-b border-white/5">
              <div className="flex gap-4">
                <div className="flex flex-col">
                  <span className="text-[9px] text-[var(--text-muted)] font-mono tracking-[0.1em]">FEED TYPE</span>
                  <span className="text-[9px] text-white font-mono tracking-[0.1em] uppercase">
                ...
                <div className="flex flex-col border-l border-white/10 pl-4">
                  <span className="text-[9px] text-[var(--text-muted)] font-mono tracking-[0.1em]">STATUS</span>
                  {/* Nothing is being received locally for an external feed — don't claim otherwise. */}
                  <span className={`text-[9px] font-mono tracking-[0.1em] ${externalOnly ? 'text-[var(--gold-primary)]' : 'text-[var(--alert-green)]'}`}>
              ...
              <div className="flex gap-3">
                ...
                {!gone && (camera.feed_url || camera.external_url || (streamType === 'iframe' && camera.stream_url)) && (
                  <a href={camera.external_url || camera.feed_url || (streamType === 'iframe' ? camera.stream_url : undefined)} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1.5 px-2 py-1 bg-white/5 hover:bg-white/10 border border-white/10 transition-colors text-[9px] font-mono text-[var(--gold-primary)] tracking-[0.1em]">
                ...
                <a href={`https://www.google.com/maps/@${camera.lat},${camera.lng},17z`} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1.5 px-2 py-1 bg-white/5 hover:bg-white/10 border border-white/10 transition-colors text-[9px] font-mono text-[var(--cyan-primary)] tracking-[0.1em]">

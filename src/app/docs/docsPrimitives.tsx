...
export function CopyButton({ value, className = '' }: { value: string; className?: string }) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1 text-[10px] font-mono tracking-[0.1em] uppercase px-2 py-1 rounded-[5px] border transition-all duration-200 ${
        copied
  ...
  return (
    <div className={`rounded-xl border border-white/[0.08] bg-[#07070D] overflow-hidden ${dense ? 'my-3' : 'my-5'}`}>
      <div className="flex items-center gap-1 px-2 h-9 border-b border-white/[0.07] bg-white/[0.015]">
        {label && !tabs.length && (
          <span className="text-[10px] font-mono tracking-[0.1em] uppercase text-[var(--text-muted)] px-2">{label}</span>
        )}
        {tabs.length > 1 ? (
          tabs.map((t, i) => (
            <button
          ...
          <span className="text-[10px] font-mono tracking-[0.1em] uppercase text-[var(--text-muted)] px-2">
            {label || tabs[0]?.label}
          </span>
        )}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <span className="text-[11px] font-mono tracking-[0.1em] uppercase" style={{ color: t.border }}>
          {title || t.label}
        </span>
      </div>
...
export function Section({
  ...
  return (
    <section id={id} className="scroll-mt-28 mb-20">
      {eyebrow && (
        <div className="text-[10px] font-mono tracking-[0.1em] uppercase text-[var(--cyan-primary)]/60 mb-2">
          {eyebrow}
        </div>
      )}
